...
export default function (instanceClass) {
  return class extends instanceClass {
    ...
    // Enable percentage-size shortcuts in the Properties Bar based on the image.
    IsOriginalSizeKnown() {
      return !!this._getFirstFrame();
    }

    GetOriginalWidth() {
      return toFiniteNumber(this._getFirstFrame()?.GetWidth?.(), 0);
    }

    GetOriginalHeight() {
      return toFiniteNumber(this._getFirstFrame()?.GetHeight?.(), 0);
    }

    // Double-click / "Edit" context menu opens the image editor, like a Sprite.
    HasDoubleTapHandler() {
      return true;
    }

    OnDoubleTap() {
      this._inst.GetObjectType?.()?.EditImage?.();
    }
    ...
    Draw(iRenderer, iDrawParams) {
      // Position the preview from the instance quad so it tracks the instance's
      // position, rotation, size and scene-graph parent transform — GetQuad()
      // already returns final hierarchy-transformed layout coordinates.
      const fallbackWidth = Math.max(0, toFiniteNumber(this._previewProps.defaultWidth, 16));
      const previewPoints =
        createPreviewPointsFromQuad(this._inst.GetQuad?.(), fallbackWidth) ??
        createPreviewPointsFromRect(this._inst.GetBoundingBox?.(), fallbackWidth);
    ...
    _getFirstFrame() {
      return this._inst.GetFirstAnimationFrame?.() ?? null;
    }

    _getEditorTexture() {
      const frame = this._getFirstFrame();
      if (!frame || typeof this.GetTexture !== "function") {
        return { texture: null, texRect: null };
      }

      const texture = this.GetTexture(frame) ?? null;
      const texRect = texture && typeof this.GetTexRect === "function" ? this.GetTexRect() : null;
    ...
    _drawMeshPreview(iRenderer, mesh) {
      ...
      const { texture, texRect } = this._getEditorTexture();
      let uvs = mesh.uvs;
      if (texture) {
        const sampling = getSamplingValue(
          getComboKey(this._previewProps.samplingMode, SAMPLING_MODE_KEYS, SAMPLING_MODE_KEYS[0])
        );
      ...
      iRenderer.DrawMesh?.(mesh.positions, uvs, mesh.indices, mesh.colors);
    }
